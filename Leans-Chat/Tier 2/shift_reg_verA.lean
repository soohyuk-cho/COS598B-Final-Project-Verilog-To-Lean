...
def shiftLeftInsert (x : Bits8) (b : Bit) : Bits8 :=
  bv8 (((x.toNat * 2) + (if b then 1 else 0)) % 256)

def step (s : State) (i : Input) : State :=
  if i.rst then
    init
  else if i.en then
    { data_out := shiftLeftInsert s.data_out i.shift_in }
  else
    { data_out := s.data_out }
...
@[simp] theorem step_shift (s : State) (b : Bit) :
    step s { rst := false, en := true, shift_in := b } =
      { data_out := shiftLeftInsert s.data_out b } := by
  simp [step]
...
theorem step_shift_example_zero :
    step { data_out := bv8 128 } { rst := false, en := true, shift_in := false } =
      { data_out := bv8 0 } := by
  native_decide
