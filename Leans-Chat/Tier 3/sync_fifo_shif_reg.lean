...
@[simp] theorem step_write_only_from_init (d : Bits8) :
    step false init { write_en := true, read_en := false, data_in := d } =
      { shift_reg := (Array.replicate DEPTH (bv8 0)).set! 0 d
        count := bv3 1
        data_out := bv8 0 } := by
  native_decide

@[simp] theorem step_read_only_singleton (x d : Bits8) :
    let s : State :=
      { shift_reg := (Array.replicate DEPTH (bv8 0)).set! 0 x
        count := bv3 1
        data_out := bv8 0 }
    step false s { write_en := false, read_en := true, data_in := d } =
      { shift_reg := (Array.replicate DEPTH (bv8 0)).set! 0 x
        count := bv3 0
        data_out := x } := by
  native_decide

@[simp] theorem step_simultaneous_read_write_singleton (x d : Bits8) :
    let s : State :=
      { shift_reg := (Array.replicate DEPTH (bv8 0)).set! 0 x
        count := bv3 1
        data_out := bv8 0 }
    step false s { write_en := true, read_en := true, data_in := d } =
      { shift_reg := (((Array.replicate DEPTH (bv8 0)).set! 0 x).set! 1 x).set! 0 d
        count := bv3 1
        data_out := x } := by
  native_decide
