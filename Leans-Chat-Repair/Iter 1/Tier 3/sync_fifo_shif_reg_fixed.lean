...
theorem step_read_only_singleton (x d : Bits8) :
    step false
      { shift_reg := (Array.replicate DEPTH (bv8 0)).set! 0 x
        count := bv3 1
        data_out := bv8 0 }
      { write_en := false, read_en := true, data_in := d } =
      { shift_reg := (Array.replicate DEPTH (bv8 0)).set! 0 x
        count := bv3 0
        data_out := x } := by
  native_decide

theorem step_simultaneous_read_write_singleton (x d : Bits8) :
    step false
      { shift_reg := (Array.replicate DEPTH (bv8 0)).set! 0 x
        count := bv3 1
        data_out := bv8 0 }
      { write_en := true, read_en := true, data_in := d } =
