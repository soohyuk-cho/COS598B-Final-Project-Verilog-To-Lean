...
instance (e : Bits13) (m : Bits53) : Decidable (isNaNUnpacked e m) := by
  unfold isNaNUnpacked
  infer_instance

instance (e : Bits13) (m : Bits53) : Decidable (isZeroUnpacked e m) := by
  unfold isZeroUnpacked
  infer_instance
...
@[simp] theorem step_reset (s : State) (i : Input) :
    step s false i = { s with phase := Phase.WAIT_REQ, rdy := false } := by
  simp [step]
