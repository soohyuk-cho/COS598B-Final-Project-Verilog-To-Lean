...
def step (s : State) (i : Input) : State :=
  if !i.rst_n then
    ...
    | FsmState.UNPACK =>
      let next_a_m := (s.a.extract 22 0 ++ BitVec.ofNat 3 0).zeroExtend 27
      let next_b_m := (s.b.extract 22 0 ++ BitVec.ofNat 3 0).zeroExtend 27
    ...
    | FsmState.SPECIAL_CASES =>
      let a_e_is_128 := s.a_e.toInt == 128
      let b_e_is_128 := s.b_e.toInt == 128
      ...
      if (a_e_is_128 && a_m_neq_0) || (b_e_is_128 && b_m_neq_0) then
        let next_z := BitVec.ofNat 1 1 ++ BitVec.ofNat 8 255 ++ BitVec.ofNat 1 1 ++ BitVec.ofNat 22 0
        { s with state := FsmState.OUT_RDY, z := next_z }
      else if a_e_is_128 then
        let next_z_base := BitVec.ofNat 1 (if s.a_s then 1 else 0) ++ BitVec.ofNat 8 255 ++ BitVec.ofNat 23 0
        let next_z := if b_e_is_128 && s.a_s != s.b_s then
                        BitVec.ofNat 1 1 ++ BitVec.ofNat 8 255 ++ BitVec.ofNat 1 1 ++ BitVec.ofNat 22 0
                      else next_z_base
        { s with state := FsmState.OUT_RDY, z := next_z }
      else if b_e_is_128 then
        let next_z := BitVec.ofNat 1 (if s.b_s then 1 else 0) ++ BitVec.ofNat 8 255 ++ BitVec.ofNat 23 0
        { s with state := FsmState.OUT_RDY, z := next_z }
      else if (a_e_is_neg127 && a_m_is_0) && (b_e_is_neg127 && b_m_is_0) then
        let next_z := BitVec.ofNat 1 (if s.a_s && s.b_s then 1 else 0) ++ ((s.b_e.extract 7 0) + BitVec.ofNat 8 127) ++ s.b_m.extract 25 3
        { s with state := FsmState.OUT_RDY, z := next_z }
      else if a_e_is_neg127 && a_m_is_0 then
        let next_z := BitVec.ofNat 1 (if s.b_s then 1 else 0) ++ ((s.b_e.extract 7 0) + BitVec.ofNat 8 127) ++ s.b_m.extract 25 3
        { s with state := FsmState.OUT_RDY, z := next_z }
      else if b_e_is_neg127 && b_m_is_0 then
        let next_z := BitVec.ofNat 1 (if s.a_s then 1 else 0) ++ ((s.a_e.extract 7 0) + BitVec.ofNat 8 127) ++ s.a_m.extract 25 3
        { s with state := FsmState.OUT_RDY, z := next_z }
      else
    ...
    | FsmState.ADD_0 =>
      let next_z_e := s.a_e
      if s.a_s == s.b_s then
        let next_pre_sum := s.a_m.zeroExtend 28 + s.b_m.zeroExtend 28
    ...
    | FsmState.ADD_1 =>
      if s.pre_sum.getLsb 27 then
        ...
        let next_z_m := s.pre_sum.extract 26 3
    ...
    | FsmState.PACK =>
      let base_exp := (s.z_e.extract 7 0) + BitVec.ofNat 8 127
      let base_mant := s.z_m.extract 22 0
      let base_sign := BitVec.ofNat 1 (if s.z_s then 1 else 0)

      let exp1 := if s.z_e.toInt == -126 && !s.z_m.getLsb 23 then BitVec.ofNat 8 0 else base_exp
      let sign1 := if s.z_e.toInt == -126 && s.z_m == BitVec.ofNat 24 0 then BitVec.ofNat 1 0 else base_sign

      let final_mant := if s.z_e.toInt > 127 then BitVec.ofNat 23 0 else base_mant
      let final_exp := if s.z_e.toInt > 127 then BitVec.ofNat 8 255 else exp1
      let final_sign := if s.z_e.toInt > 127 then BitVec.ofNat 1 (if s.z_s then 1 else 0) else sign1
