...
def arrayGetD {α : Type} (arr : Array α) (idx : Nat) (defVal : α) : α :=
  if h : idx < arr.size then arr[idx] else defVal
...
def step {dw cw cdbw w_idx : Nat} (s : State w_idx) (i : Input dw cw cdbw w_idx) : State w_idx := Id.run do
  ...
  for j in [0:cdbw] do
    if arrayGetD i.cdb_valid j false then
      ...
      let mut entry := next_entries.get! idx_nat
      if entry.valid then
        entry := { entry with completed := true, result := arrayGetD i.cdb_data j 0 }
        next_entries := next_entries.set! idx_nat entry
  ...
  if i.cdb_branch_complete then
    ...
    if entry.valid && entry.branch then
      entry := { entry with completed := true, branch_mispredicted := i.cdb_branch_mispredicted, branch_correct_pc := i.cdb_branch_correct_pc }
      next_entries := next_entries.set! idx_nat entry
  ...
  if i.load_complete_valid then
    ...
    if entry.valid && entry.mem_read then
      entry := { entry with completed := true }
      next_entries := next_entries.set! idx_nat entry
  ...
  for j in [0:cw] do
    ...
    let entry := next_entries.get! idx_nat
  ...
  if commit_misprediction then
    ...
    for k in [0:(2 ^ w_idx)] do
      ...
      if k_bv == misp_idx_bv then
         let entry := next_entries.get! k
         next_entries := next_entries.set! k { entry with valid := false, completed := false }
      else if (next_entries.get! k).valid then
    ...
    for j in [0:commit_count] do
      ...
      let idx_nat := (get_idx check_ptr).toNat % (2 ^ w_idx)
      let entry := next_entries.get! idx_nat
      next_entries := next_entries.set! idx_nat { entry with valid := false, completed := false }
    ...
    for j in [0:dw] do
      ...
      if can_disp then
        ...
        next_entries := next_entries.set! alloc_idx_nat new_entry
...
def out {dw cw cdbw w_idx : Nat} (s : State w_idx) (i : Input dw cw cdbw w_idx) : Output dw cw w_idx := Id.run do
  let mut out_dispatch_rob_idx := Array.mkArray dw 0
  ...
  for j in [0:dw] do
    ...
    let can_disp := arrayGetD i.dispatch_valid j false && has_space
    if can_disp then
      out_dispatch_rob_idx := out_dispatch_rob_idx.set! j (get_idx curr_tail)
      out_dispatch_rob_valid := out_dispatch_rob_valid.set! j true
      curr_tail := tentative_tail

  let mut out_commit_valid := Array.mkArray cw false
  let mut out_commit_arch_rd := Array.mkArray cw 0
  let mut out_commit_phys_rd := Array.mkArray cw 0
  let mut out_commit_old_phys_rd := Array.mkArray cw 0
  ...
  let mut out_commit_rob_idx := Array.mkArray cw 0
  let mut out_commit_pc := Array.mkArray cw 0

  let mut out_store_commit_valid := Array.mkArray cw false
  let mut out_store_commit_rob_idx := Array.mkArray cw 0
  ...
  for j in [0:cw] do
    let check_ptr := s.head_ptr + BitVec.ofNat (w_idx + 1) j
    let idx_nat := (get_idx check_ptr).toNat % (2 ^ w_idx)
    let entry := s.entries.get! idx_nat
    let is_misp := entry.branch && entry.branch_mispredicted
    let can_commit := entry.valid && entry.completed && !commit_misprediction && (check_ptr != s.tail_ptr)

    if can_commit then
      out_commit_valid := out_commit_valid.set! j true
      out_commit_arch_rd := out_commit_arch_rd.set! j entry.arch_rd
      out_commit_phys_rd := out_commit_phys_rd.set! j entry.phys_rd
      out_commit_old_phys_rd := out_commit_old_phys_rd.set! j entry.old_phys_rd
      out_commit_reg_write := out_commit_reg_write.set! j entry.reg_write
      out_commit_store := out_commit_store.set! j entry.mem_write
      out_commit_load := out_commit_load.set! j entry.mem_read
      out_commit_rob_idx := out_commit_rob_idx.set! j (get_idx check_ptr)
      out_commit_pc := out_commit_pc.set! j entry.pc

      out_store_commit_valid := out_store_commit_valid.set! j entry.mem_write
      out_store_commit_rob_idx := out_store_commit_rob_idx.set! j (get_idx check_ptr)
