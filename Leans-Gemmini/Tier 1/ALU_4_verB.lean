...
def eval (input : Inputs) : Outputs :=
  let add_result := input.lhs + input.rhs
  let sub_result := input.lhs - input.rhs
  let and_result := input.lhs &&& input.rhs
  let or_result  := input.lhs ||| input.rhs
