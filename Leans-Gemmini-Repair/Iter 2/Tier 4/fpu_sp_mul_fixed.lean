...
def step (s : State) (i : Input) : State :=
  if !i.rst_n then
    ...
    | FsmState.SPECIAL_CASES =>
      ...
      let nan_z := BitVec.ofNat 32 0xFFC00000
      let sign_diff := if s.a_s != s.b_s then BitVec.ofNat 32 0x80000000 else BitVec.ofNat 32 0
      let z_inf := sign_diff + BitVec.ofNat 32 0x7F800000
      let z_zero := sign_diff

      if (a_e_is_128 && a_m_nz) || (b_e_is_128 && b_m_nz) then
        { s with state := FsmState.OUT_RDY, z := nan_z }
      else if a_e_is_128 then
        let z_final := if b_e_is_neg127 && b_m_z then nan_z else z_inf
        { s with state := FsmState.OUT_RDY, z := z_final }
      else if b_e_is_128 then
        let z_final := if a_e_is_neg127 && a_m_z then nan_z else z_inf
        { s with state := FsmState.OUT_RDY, z := z_final }
      else if a_e_is_neg127 && a_m_z then
        { s with state := FsmState.OUT_RDY, z := z_zero }
      else if b_e_is_neg127 && b_m_z then
        { s with state := FsmState.OUT_RDY, z := z_zero }
      else
        let next_a_e := if a_e_is_neg127 then (BitVec.ofNat 10 0 - BitVec.ofNat 10 126) else s.a_e
        let next_a_m := if a_e_is_neg127 then s.a_m else s.a_m + BitVec.ofNat 24 0x800000
        let next_b_e := if b_e_is_neg127 then (BitVec.ofNat 10 0 - BitVec.ofNat 10 126) else s.b_e
        let next_b_m := if b_e_is_neg127 then s.b_m else s.b_m + BitVec.ofNat 24 0x800000
    ...
    | FsmState.MULTIPLY_0 =>
      ...
      let next_product := BitVec.zeroExtend 48 s.a_m * BitVec.zeroExtend 48 s.b_m
      { s with state := FsmState.MULTIPLY_1, z_s := next_z_s, z_e := next_z_e, product := next_product }

    | FsmState.MULTIPLY_1 =>
      let next_z_m := BitVec.zeroExtend 24 (s.product.extractLsb 47 24)
      let next_guard := s.product.getLsb 23
      let next_round_bit := s.product.getLsb 22
      let next_sticky := BitVec.zeroExtend 22 (s.product.extractLsb 21 0) != BitVec.ofNat 22 0
    ...
    | FsmState.PACK =>
      let sign_bit_bv := if s.z_s then BitVec.ofNat 32 0x80000000 else BitVec.ofNat 32 0
      let exp_add := BitVec.zeroExtend 10 (s.z_e.extractLsb 9 0) + BitVec.ofNat 10 127
      let exp_val_bv := (BitVec.zeroExtend 32 (exp_add.extractLsb 7 0)) <<< 23
      ...
      let next_z2 := if s.z_e.toInt == -126 && !s.z_m.getLsb 23 then
                       next_z &&& BitVec.ofNat 32 0x807FFFFF
                     else
                       next_z

      let next_z3 := if s.z_e.toInt > 127 then
                       sign_bit_bv + BitVec.ofNat 32 0x7F800000
                     else
                       next_z2
...
@[simp] theorem step_hold_WAIT_REQ (rdy : Bool) (res a b z : BitVec 32)
  (a_m b_m z_m : BitVec 24) (a_e b_e z_e : BitVec 10)
  (a_s b_s z_s guard round_bit sticky : Bool) (prod : BitVec 48) (d1 d2 : BitVec 32) :
  step { state := FsmState.WAIT_REQ, rdy := rdy, result := res, a := a, b := b, z := z,
         a_m := a_m, b_m := b_m, z_m := z_m, a_e := a_e, b_e := b_e, z_e := z_e,
         a_s := a_s, b_s := b_s, z_s := z_s, guard := guard, round_bit := round_bit,
         sticky := sticky, product := prod }
       { rst_n := true, din1 := d1, din2 := d2, dval := false } =
  { state := FsmState.WAIT_REQ, rdy := false, result := res, a := a, b := b, z := z,
    a_m := a_m, b_m := b_m, z_m := z_m, a_e := a_e, b_e := b_e, z_e := z_e,
    a_s := a_s, b_s := b_s, z_s := z_s, guard := guard, round_bit := round_bit,
    sticky := sticky, product := prod } := by
  rfl
