...
def step (s : State) (i : Input) : State :=
  if !i.rst_n then
    ...
    | FsmState.SPECIAL_CASES =>
      let a_e_1024 := s.a_e == BitVec.ofNat 13 1024
      let b_e_1024 := s.b_e == BitVec.ofNat 13 1024
      ...
      let nan_z := (BitVec.ofNat 64 0).setBit 63 |>.setLsb 62 52 (BitVec.ofNat 11 2047) |>.setBit 51
      ...
      if (a_e_1024 && a_m_nz) || (b_e_1024 && b_m_nz) then
        ...
        let z_base := (BitVec.ofNat 64 0).setLsb 63 63 (if s.a_s != s.b_s then 1 else 0) |>.setLsb 62 52 (BitVec.ofNat 11 2047)
        let z_final := if b_e_n1023 && b_m_z then nan_z else z_base
        { s with state := FsmState.OUT_RDY, z := z_final }
      else if b_e_1024 then
        let z_final := (BitVec.ofNat 64 0).setLsb 63 63 (if s.a_s != s.b_s then 1 else 0) |>.setLsb 62 52 (BitVec.ofNat 11 0)
        { s with state := FsmState.OUT_RDY, z := z_final }
      else if a_e_n1023 && a_m_z then
        let z_base := (BitVec.ofNat 64 0).setLsb 63 63 (if s.a_s != s.b_s then 1 else 0) |>.setLsb 62 52 (BitVec.ofNat 11 0)
        let z_final := if b_e_n1023 && b_m_z then nan_z else z_base
        { s with state := FsmState.OUT_RDY, z := z_final }
      else if b_e_n1023 && b_m_z then
        let z_final := (BitVec.ofNat 64 0).setLsb 63 63 (if s.a_s != s.b_s then 1 else 0) |>.setLsb 62 52 (BitVec.ofNat 11 2047)
        { s with state := FsmState.OUT_RDY, z := z_final }
      else
        let next_a_e := if a_e_n1023 then (BitVec.ofNat 13 0 - BitVec.ofNat 13 1022) else s.a_e
        let next_a_m := if a_e_n1023 then s.a_m else s.a_m.setBit 52
        let next_b_e := if b_e_n1023 then (BitVec.ofNat 13 0 - BitVec.ofNat 13 1022) else s.b_e
    ...
    | FsmState.DIVIDE_1 =>
      ...
      let next_remainder := (s.remainder <<< 1) ||| (if s.dividend.getLsb 108 then BitVec.ofNat 109 1 else BitVec.ofNat 109 0)
    ...
    | FsmState.DIVIDE_3 =>
      { s with
        ...
        z_m := BitVec.zeroExtend 53 (s.quotient.extractLsb 55 3),
