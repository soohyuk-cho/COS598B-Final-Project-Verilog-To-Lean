...
def init {w_idx : Nat} : State w_idx := {
  entries := mkArray (2 ^ w_idx) initEntry,
  head_ptr := 0,
  tail_ptr := 0,
  count := 0
}
...
def arraySetD {α : Type} (arr : Array α) (idx : Nat) (val : α) : Array α :=
  if h : idx < arr.size then arr.set ⟨idx, h⟩ val else arr
...
def out {dw cw cdbw w_idx : Nat} (s : State w_idx) (i : Input dw cw cdbw w_idx) : Output dw cw w_idx := Id.run do
  let mut out_dispatch_rob_idx := mkArray dw (BitVec.ofNat (w_idx + 1) 0)
  let mut out_dispatch_rob_valid := mkArray dw false
  let mut curr_tail := s.tail_ptr
  ...
  let mut out_commit_valid := mkArray cw false
  let mut out_commit_arch_rd := mkArray cw (BitVec.ofNat 5 0)
  let mut out_commit_phys_rd := mkArray cw (BitVec.ofNat 6 0)
  let mut out_commit_old_phys_rd := mkArray cw (BitVec.ofNat 6 0)
  let mut out_commit_reg_write := mkArray cw false
  let mut out_commit_store := mkArray cw false
  let mut out_commit_load := mkArray cw false
  let mut out_commit_rob_idx := mkArray cw (BitVec.ofNat (w_idx + 1) 0)
  let mut out_commit_pc := mkArray cw (BitVec.ofNat 32 0)

  let mut out_store_commit_valid := mkArray cw false
  let mut out_store_commit_rob_idx := mkArray cw (BitVec.ofNat (w_idx + 1) 0)
