...
def step (s : State) (i : Input) : State :=
  if !i.rst_n then
    ...
    | FsmState.UNPACK =>
      ...
      let next_a_e := (BitVec.zeroExtend 10 (s.a.extractLsb 30 23)) - 127
      let next_b_e := (BitVec.zeroExtend 10 (s.b.extractLsb 30 23)) - 127
    ...
    | FsmState.SPECIAL_CASES =>
      ...
      let nan_z := (BitVec.ofNat 32 0).setBit 31 |>.setLsb 30 23 255 |>.setBit 22
      ...
      if (a_e_128 && a_m_nz) || (b_e_128 && b_m_nz) then
        ...
        let z_base := BitVec.ofNat 32 0 |>.setLsb 31 (if s.a_s != s.b_s then 1 else 0) |>.setLsb 30 23 255
        let z_final := if b_e_n127 && b_m_z then nan_z else z_base
        { s with state := FsmState.OUT_RDY, z := z_final }
      else if b_e_128 then
        let z_final := BitVec.ofNat 32 0 |>.setLsb 31 (if s.a_s != s.b_s then 1 else 0) |>.setLsb 30 23 0
        { s with state := FsmState.OUT_RDY, z := z_final }
      else if a_e_n127 && a_m_z then
        let z_base := BitVec.ofNat 32 0 |>.setLsb 31 (if s.a_s != s.b_s then 1 else 0) |>.setLsb 30 23 0
        let z_final := if b_e_n127 && b_m_z then nan_z else z_base
        { s with state := FsmState.OUT_RDY, z := z_final }
      else if b_e_n127 && b_m_z then
        let z_final := BitVec.ofNat 32 0 |>.setLsb 31 (if s.a_s != s.b_s then 1 else 0) |>.setLsb 30 23 255
        { s with state := FsmState.OUT_RDY, z := z_final }
      else
        let next_a_e := if a_e_n127 then (BitVec.ofNat 10 0 - 126) else s.a_e
        let next_a_m := if a_e_n127 then s.a_m else s.a_m.setBit 23
        let next_b_e := if b_e_n127 then (BitVec.ofNat 10 0 - 126) else s.b_e
        let next_b_m := if b_e_n127 then s.b_m else s.b_m.setBit 23
    ...
    | FsmState.DIVIDE_1 =>
      ...
      let next_remainder := (s.remainder <<< 1) | (if s.dividend.getLsb 50 then 1 else 0)
    ...
    | FsmState.ROUND =>
      if s.guard && (s.round_bit || s.sticky || s.z_m.getLsb 0) then
        ...
        let next_z_e := if s.z_m == 0xffffff then s.z_e + 1 else s.z_e
    ...
    | FsmState.PACK =>
      ...
      next_z := next_z.setBit 31 s.z_s
      next_z := next_z.setLsb 30 23 (BitVec.zeroExtend 8 (s.z_e + 127))
      next_z := next_z.setLsb 22 0 (BitVec.zeroExtend 23 (s.z_m.extractLsb 22 0))

      if s.z_e.toInt == -126 && !s.z_m.getLsb 23 then
        next_z := next_z.setLsb 30 23 0

      if s.z_e.toInt > 127 then
        next_z := BitVec.ofNat 32 0 |>.setBit 31 s.z_s |>.setLsb 30 23 255
