...
def step {dw cw cdbw w_idx : Nat} (s : State w_idx) (i : Input dw cw cdbw w_idx) : State w_idx := Id.run do
  ...
  for j in [0:cdbw] do
    if arrayGetD i.cdb_valid j false then
      ...
      let entry := next_entries[idx_nat]!
      if entry.valid then
        next_entries := next_entries.set! idx_nat { entry with completed := true, result := arrayGetD i.cdb_data j 0 }

  if i.cdb_branch_complete then
    let idx_nat := (get_idx i.cdb_branch_rob_idx).toNat % (2 ^ w_idx)
    let entry := next_entries[idx_nat]!
    if entry.valid && entry.branch then
      next_entries := next_entries.set! idx_nat { entry with completed := true, branch_mispredicted := i.cdb_branch_mispredicted, branch_correct_pc := i.cdb_branch_correct_pc }

  if i.load_complete_valid then
    let idx_nat := (get_idx i.load_complete_rob_idx).toNat % (2 ^ w_idx)
    let entry := next_entries[idx_nat]!
    if entry.valid && entry.mem_read then
      next_entries := next_entries.set! idx_nat { entry with completed := true }
  ...
  if commit_misprediction then
    ...
    for k in [0:(2 ^ w_idx)] do
      ...
      if k_bv == misp_idx_bv then
         next_entries := next_entries.set! k { next_entries[k]! with valid := false, completed := false }
      else if next_entries[k]!.valid then
    ...
    for j in [0:commit_count] do
      let check_ptr := s.head_ptr + BitVec.ofNat (w_idx + 1) j
      let idx_nat := (get_idx check_ptr).toNat % (2 ^ w_idx)
      next_entries := next_entries.set! idx_nat { next_entries[idx_nat]! with valid := false, completed := false }
