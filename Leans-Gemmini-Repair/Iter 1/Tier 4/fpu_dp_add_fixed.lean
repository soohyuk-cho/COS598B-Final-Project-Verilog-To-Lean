  ...
  if !i.rst_n then
    ...
    | FsmState.SPECIAL_CASES =>
      ...
      if (a_e_is_1024 && a_m_neq_0) || (b_e_is_1024 && b_m_neq_0) then
        let next_z := BitVec.ofNat 64 0 |>.setLsb 63 true |>.setLsb 62 true |>.setLsb 51 true
        -- z[62:52] = 2047, z[51]=1, z[63]=1
        let next_z_final := (BitVec.ofNat 64 0).setBit 63 |>.setLsb 62 52 (BitVec.ofNat 11 2047) |>.setBit 51
        { s with state := FsmState.OUT_RDY, z := next_z_final }
      else if a_e_is_1024 then
        let sign := s.a_s
        let next_z_base := BitVec.ofNat 64 0 |>.setLsb 63 sign |>.setLsb 62 52 (BitVec.ofNat 11 2047)
        ...
        let sign := s.b_s
        let next_z := BitVec.ofNat 64 0 |>.setLsb 63 sign |>.setLsb 62 52 (BitVec.ofNat 11 2047)
        { s with state := FsmState.OUT_RDY, z := next_z }
      else if (a_e_is_neg1023 && a_m_is_0) && (b_e_is_neg1023 && b_m_is_0) then
        let sign := s.a_s && s.b_s
        let next_z := BitVec.ofNat 64 0 |>.setLsb 63 sign |>.setLsb 62 52 (BitVec.zeroExtend 11 (s.b_e + BitVec.ofNat 13 1023)) |>.setLsb 51 0 (BitVec.zeroExtend 52 (s.b_m >>> 3))
        { s with state := FsmState.OUT_RDY, z := next_z }
      else if a_e_is_neg1023 && a_m_is_0 then
        let next_z := BitVec.ofNat 64 0 |>.setLsb 63 s.b_s |>.setLsb 62 52 (BitVec.zeroExtend 11 (s.b_e + BitVec.ofNat 13 1023)) |>.setLsb 51 0 (BitVec.zeroExtend 52 (s.b_m >>> 3))
        { s with state := FsmState.OUT_RDY, z := next_z }
      else if b_e_is_neg1023 && b_m_is_0 then
        let next_z := BitVec.ofNat 64 0 |>.setLsb 63 s.a_s |>.setLsb 62 52 (BitVec.zeroExtend 11 (s.a_e + BitVec.ofNat 13 1023)) |>.setLsb 51 0 (BitVec.zeroExtend 52 (s.a_m >>> 3))
        { s with state := FsmState.OUT_RDY, z := next_z }
      else
    ...
    | FsmState.PACK =>
      ...
      next_z := next_z.setLsb 62 52 (BitVec.zeroExtend 11 (s.z_e + BitVec.ofNat 13 1023))
      ...
      if s.z_e.toInt > 1023 then
        next_z := next_z.setLsb 51 0 (BitVec.ofNat 52 0) |>.setLsb 62 52 (BitVec.ofNat 11 2047) |>.setBit 63 s.z_s
