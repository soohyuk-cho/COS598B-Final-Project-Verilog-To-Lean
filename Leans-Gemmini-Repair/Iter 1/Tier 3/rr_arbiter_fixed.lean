...
def step {n : Nat} (s : State n) (i : Input n) : State n :=
  if !i.rst_n then
    ...
    { grant := grant_next
    , rotate_ptr := if grant_next != BitVec.ofNat n 0 then next_ptr else s.rotate_ptr }
...
@[simp] theorem step_update {n : Nat} (s : State n) (r : BitVec n) :
  ...
  { grant := g, rotate_ptr := if g != BitVec.ofNat n 0 then p else s.rotate_ptr } := by
  simp [step]
