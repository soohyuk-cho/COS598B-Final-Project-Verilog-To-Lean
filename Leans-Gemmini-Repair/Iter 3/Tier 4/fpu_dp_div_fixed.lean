...
def step (s : State) (i : Input) : State :=
  if !i.rst_n then
    ...
    | FsmState.SPECIAL_CASES =>
      ...
      let diff_s := if s.a_s != s.b_s then (BitVec.ofNat 64 1 <<< 63) else BitVec.ofNat 64 0
      let z_base := diff_s ||| (BitVec.ofNat 64 2047 <<< 52)
      let z_zero := diff_s
      ...
      if (a_e_1024 && a_m_nz) || (b_e_1024 && b_m_nz) then
        ...
        let z_final := if b_e_n1023 && b_m_z then nan_z else z_zero
        { s with state := FsmState.OUT_RDY, z := z_final }
    ...
    | FsmState.PACK =>
      ...
      let z2 := if s.z_e.toInt > 1023 then
                  sign_bit_bv ||| (BitVec.ofNat 64 2047 <<< 52)
                else
                  z1

      { s with state := FsmState.OUT_RDY, z := z2 }
